...
When we write generic structs, the compiler is actually generating multiple different structs during compile time with each concrete type we use. In our example, the compiler will generate types that look like the following.
